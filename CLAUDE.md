...
| [`design-system-TAD.md`](docs/design-system/design-system-TAD.md) | Design system technical architecture — topology, packages, substrate, theme layer, consumption contracts, release gates | Low — changes when architecture changes |
...
- **Draft-before-write.** When authoring an ADR, the assistant drafts its full prose and presents it in the conversation for the owner to review and refine — the file is not created until the owner approves the text. Nothing is written ahead of that approval: no ADR file, no index row, no edit to the source decision backlog. (The index row is added in the same change as the file — see *Index discipline*.)
- **Self-contained records.** An ADR must read correctly on its own, indefinitely. State the substance of the question it resolves in the ADR's own Context — do not anchor the record to transient working state (e.g. "decision-backlog item N asked…") or reconcile against backlog items in the body, since those rows are removed once the decision is recorded.
- **Requirement references.** Cite the PRD requirement an ADR serves by section number, paired with the concept it names, so the reference survives a future PRD renumber (git preserves the PRD's state at the ADR's date). Do not forward-reference a section that does not yet exist — e.g. a requirement being added in the same change because the ADR surfaced it; name it generically ("a new PRD feature line") and let the PRD and backlog carry the number (see §8).
- **Backlog resolution.** When a decision-backlog question is resolved into an accepted ADR, remove its row from the backlog rather than leaving it duplicated — but do not renumber the remaining rows; renumbering happens only during deliberate housekeeping.
- **Decision authority — hard gate.** The repository owner is the sole decision-maker for every ADR's Decision. An AI assistant working in this repository may research and present alternatives, lay out tradeoffs, and draft an ADR's prose once a decision has actually been made — but does not choose the decision, does not write a Decision line unprompted, and does not present a completed ADR as a fait accompli for the owner to simply accept or reject after the fact. See §8 for how this fits the broader PRD → ADR → TAD sequence.
...
- **Don't draft, propose, or number an ADR as a side effect of other work.** If a decision point surfaces while doing something else — writing a PRD, preparing a handoff, answering an unrelated question — flag it explicitly and stop there. Do not fold a Decision into whatever artifact is already in progress. This has happened before in this repository's history and is the specific failure mode §5 and §8 exist to prevent.
...
*CLAUDE.md v1.3 — buzonas.dev-governance Repository*
